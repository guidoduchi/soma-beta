    ...
    *,
    ...
    with UnitOfWork(factory) as uow:
        uow.connection.execute(
            ...
            ") VALUES (?,?,'manual','PROFILE_V1','HEADERS_V1','VOCAB_V1','PARSER_V1','mismatched.xlsx',100,1,?,200,?,'waiting_review',0,1,1,1,1)",
            (run_id, run_source_family, chronology_kind, "1" * 64),
        )
        uow.connection.execute(
...
def test_rfc_evidence_rejects_observation_whose_import_run_has_another_source_family(initialized_database) -> None:
    ...
    field_id = _seed_mismatched_field(
        factory,
