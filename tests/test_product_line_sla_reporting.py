...
class _FailingSectionContributor:
    def section_descriptor(self) -> ReportSectionDescriptor:
        return ReportSectionDescriptor(
            section_kind="f016_failure_probe",
            schema_name="F016FailureProbeV1",
            schema_version=1,
            ordinal=1,
            payload_contract=ObjectContract(
                name="F016FailureProbeV1",
                version=1,
    ...
    def stream_snapshot_rows(self, snapshot, report_request, as_of_utc):
        ...
        raise RuntimeError("injected F016 contributor failure")


def test_f016_contributor_failure_blocks_seal_and_recovery_fails_staging_safely(
    initialized_database,
) -> None:
    factory = _factory(initialized_database)
    month_start, _month_end = canonical_month_bounds(MONTH)
    ...
    claim = jobs.claim_next(new_uuid4(), utc_epoch_seconds())
    assert claim is not None
    worker = SlaReportGenerationWorker(
        factory,
        section_registry=ReportSectionContributorRegistry(
            (_FailingSectionContributor(),)
        ),
    )

    with pytest.raises(RuntimeError, match="injected F016"):
        worker.run_to_completion(claim)

    with ReadSnapshot(factory) as snapshot:
        interrupted = ReportRepository.get_attempt(snapshot.connection, report_id)
        assert interrupted is not None
        assert interrupted.state == "staging"
        assert interrupted.snapshot_hash is None
        assert ReportRepository.counts(snapshot.connection, report_id) == (0, 0, 0, 0)

    recovery_run = new_uuid4()
    now = utc_epoch_seconds()
    summary = jobs.recover_stale_claims(recovery_run, now)
    assert summary.retry_wait_count == 1
    recovered_claim = jobs.claim_next(recovery_run, now)
    assert recovered_claim is not None
    assert recovered_claim.job_id == claim.job_id
    assert recovered_claim.attempt_ordinal == claim.attempt_ordinal + 1

    assert worker.run_to_completion(recovered_claim) == report_id

    with ReadSnapshot(factory) as snapshot:
        failed = ReportRepository.get_attempt(snapshot.connection, report_id)
        assert failed is not None
        assert failed.state == "failed"
        assert failed.failure_code == "SLA_REPORT_SNAPSHOT_INTERRUPTED"
        assert failed.snapshot_hash is None
        assert failed.snapshot_member_count == 0
        assert failed.snapshot_cohort_count == 0
        assert failed.snapshot_section_row_count == 0
        ...
        assert tuple(job) == ("failed",)


def test_f019_seal_count_mismatch_leaves_attempt_staging_and_unreceipted(
    initialized_database,
) -> None:
    factory = _factory(initialized_database)
    sr = ServiceRequestService(factory).create_manual_service_request(
        command_id=new_uuid4(),
        official_sr_no="99000019",
    )
    attempt, claim, jobs, token = _start_and_claim(factory)
    report_id = str(attempt["report_attempt_id"])
    worker = SlaReportWorkerService(factory)
    ...
    with pytest.raises(SomaError) as mismatch:
        worker.seal(
            ...
            snapshot_generation_token=token,
            expected_attempt_revision=2,
            final_member_count=2,
            final_cohort_count=0,
            final_section_row_count=0,
            snapshot_hash=snapshot_hash,
        )
    assert mismatch.value.code == "SLA_REPORT_SNAPSHOT_MISMATCH"

    with ReadSnapshot(factory) as snapshot:
        current = ReportRepository.get_attempt(snapshot.connection, report_id)
        assert current is not None
        assert current.state == "staging"
        assert current.revision == 2
        assert current.snapshot_hash is None
        assert ReportRepository.counts(snapshot.connection, report_id) == (1, 0, 0, 0)
        assert snapshot.connection.execute(
            "SELECT COUNT(*) FROM command_receipts WHERE command_id=?",
            (seal_command,),
        ).fetchone()[0] == 0
...
def test_f024_completion_audit_failure_rolls_back_completed_state_and_receipt(
    initialized_database,
    monkeypatch,
) -> None:
    factory = _factory(initialized_database)
    ...
    generating = worker.mark_generating(
        command_id=new_uuid4(),
        report_attempt_id=report_id,
        expected_attempt_revision=int(sealed["resulting_attempt_revision"]),
        snapshot_hash=snapshot_hash,
    )
    candidate = ".f024-candidate.xlsx"
    verifying = worker.mark_verifying(
        command_id=new_uuid4(),
        report_attempt_id=report_id,
    ...
    def fail_audit(*_args, **_kwargs):
        raise SomaError(
            ...
            "injected F024 completion audit failure",
        )
    ...
    with pytest.raises(SomaError) as failure:
        worker.complete(
            ...
            artifact_completion={
                ...
                "snapshot_hash": snapshot_hash,
                "candidate_filename": candidate,
                "artifact_filename": "f024-final.xlsx",
    ...
    with ReadSnapshot(factory) as snapshot:
        ...
        assert tuple(job) == ("running",)


def test_f025_partial_failure_cleanup_rolls_back_every_delete_and_terminalization(
    initialized_database,
    monkeypatch,
) -> None:
    factory = _factory(initialized_database)
    sr = ServiceRequestService(factory).create_manual_service_request(
        command_id=new_uuid4(),
        official_sr_no="99000025",
    )
    attempt, _claim, _jobs, token = _start_and_claim(factory)
    report_id = str(attempt["report_attempt_id"])
    worker = SlaReportWorkerService(factory)
    ...
    def partial_cleanup_then_fail(connection, target_report_id):
        connection.execute(
            "DELETE FROM sla_report_member_snapshots WHERE report_attempt_id=?",
            (target_report_id,),
        )
        raise RuntimeError("injected F025 mid-cleanup failure")

    monkeypatch.setattr(
        ReportRepository,
        "cleanup_staging",
        staticmethod(partial_cleanup_then_fail),
    )
    fail_command = new_uuid4()
    with pytest.raises(RuntimeError, match="injected F025"):
        worker.fail(
            command_id=fail_command,
            report_attempt_id=report_id,
            expected_attempt_revision=2,
            failure_code="SLA_REPORT_ARTIFACT_WRITE_FAILED",
        )
