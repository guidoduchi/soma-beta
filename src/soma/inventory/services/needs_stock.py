            ...
            def apply(inner: UnitOfWork):
                ...
                return AuditEventInput(
                    ...
                    payload={
                        ...
                        "allocation_id": None,
                        "spare_need_id": None,
                        "resulting_unit_revision": revision,
            ...
            def apply(inner: UnitOfWork):
                ...
                return AuditEventInput(
                    ...
                    payload={
                        "spare_part_unit_id": spare_part_unit_id,
                        "event_kind": "RESERVE",
                        "local_tracking_id": None if current[1] is None else str(current[1]),
                        "task_id": task_id,
                        "allocation_id": allocation_id,
                        "spare_need_id": need_id,
                        "resulting_unit_revision": resulting_unit_revision,
                        "allocation_revision": allocation_revision,
                        "bom_fingerprint": self._fingerprint(str(current[3])),
                        "reason_category": None,
                    },
                    resulting_event_refs=(
                        AuditResultRef("spare_part_unit", spare_part_unit_id),
            ...
            def apply(inner: UnitOfWork):
                ...
                return AuditEventInput(
                    ...
                    payload={
                        ...
                        "event_kind": "RELEASE",
                        "local_tracking_id": None if after[1] is None else str(after[1]),
                        "task_id": str(current[0]),
                        "allocation_id": allocation_id,
                        "spare_need_id": None if current[2] is None else str(current[2]),
                        "resulting_unit_revision": resulting_unit_revision,
                        "allocation_revision": base_revision,
                        "bom_fingerprint": self._fingerprint(str(after[3])),
                        "reason_category": reason,
                    },
                    resulting_event_refs=(
        ...
        def prepare(uow: UnitOfWork) -> PreparedMutation:
            ...
            def apply(inner: UnitOfWork):
                ...
                unit_audit = AuditEventInput(
                    ...
                    payload={
                        "spare_part_unit_id": spare_part_unit_id,
                        "event_kind": "LOCAL_SELECTION" if task is None else "RESERVE",
                        "local_tracking_id": None if after[1] is None else str(after[1]),
                        "task_id": task,
                        "allocation_id": allocation_id,
                        "spare_need_id": spare_need_id,
