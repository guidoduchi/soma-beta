...
from soma.ticket_import.providers.sr_contact_reconciliation import TicketImportSrContactReconciliationProvider
...
_SR_CONTACT_PROPOSAL_ROLES = {
    "sr_contact_reconciliation": "customer_contact",
    "sr_current_handler_reconciliation": "current_handler_reference",
}
...
class ProposalDecisionService:
    ...
    def _prepare_sr_contact_accept(
        ...
        *,
        ...
        reference_role = _SR_CONTACT_PROPOSAL_ROLES.get(proposal.proposal_kind)
        if (
        ...
        if current_contact_id == candidate.contact_id:
        ...
        def apply(inner: UnitOfWork):
            if reference_role == "customer_contact":
                owner_result = self._sr_import_mutations.set_customer_contact_from_review(inner, mutation)
            else:
