...
_OPERATION_PROJECTION_PROFILE = "SOMA_RFC_ARCHIVE_OPERATION_PROJECTION_V1"
...
def _validate_scope_kind(scope_kind: object) -> str:
...
class RfcArchiveOperationMember:
    ...
    def from_response(cls, value: Any) -> "RfcArchiveOperationMember":
        ...
        current_state = value["current_archive_state"]
        ...
        if (
            ...
            or type(restore_eligible) is not bool
        ):
...
class RfcArchiveOperation:
    ...
    def from_response(cls, value: Any) -> "RfcArchiveOperation":
        ...
        for member in members:
            expected_eligible = (
                member.current_archive_state == "archived"
                and member.latest_event_type == "archived"
                and member.latest_event_origin_archive_operation_id == operation_id
            )
            if member.restore_eligible != expected_eligible:
                raise ValidationError("RfcArchiveOperationV1 restore eligibility is invalid")
        continuation = value["continuation"]
        if continuation is not None:
            last = _validate_operation_cursor(
                continuation,
...
def _validate_preview_cursor(
    ...
    *,
    ...
    return ordinal, _request_uuid(member_id, field="RFC archive preview cursor member identity")


def _operation_filter_fingerprint(*, operation_id: str, projection_fingerprint: str) -> str:
    return sha256_canonical_json(
        {
...
def _operation_cursor(
    *,
    operation_id: str,
    projection_fingerprint: str,
    member: RfcArchiveOperationMember,
) -> dict[str, object]:
    return {
        "version": 1,
        ...
        "filter_fingerprint": _operation_filter_fingerprint(
            operation_id=operation_id,
            projection_fingerprint=projection_fingerprint,
        ),
        "null_order": _NULL_ORDER,
...
def _validate_operation_cursor(
    cursor: Any,
    *,
    operation_id: str,
    projection_fingerprint: str,
    ...
    expected_filter = _operation_filter_fingerprint(
        operation_id=operation_id,
        projection_fingerprint=projection_fingerprint,
    )
    if cursor["filter_fingerprint"] != expected_filter:
...
def archive_operation_projection_fingerprint(
    *,
    rfc_archive_operation_id: str,
    scope_kind: str,
    scope_fingerprint: str,
    members: Iterable[RfcArchiveOperationMember],
) -> str:
    return sha256_canonical_json(
        {
            "schema": _OPERATION_PROJECTION_PROFILE,
            "rfc_archive_operation_id": rfc_archive_operation_id,
            "scope_kind": scope_kind,
            "scope_fingerprint": scope_fingerprint,
            "members": [member.to_response() for member in members],
        }
    )
...
class RfcArchiveQueryService:
    ...
    def _stream_operation_projection_fingerprint(
        self,
        connection: Any,
        *,
        operation_id: str,
        ...
        for row in rows:
            member = _operation_member_from_row(row, operation_id)
            if not first:
                digest.update(b",")
            first = False
            digest.update(canonical_json_bytes(member.to_response()))
            exact_count += 1
        digest.update(b'],"rfc_archive_operation_id":')
        digest.update(canonical_json_bytes(operation_id))
        digest.update(b',"schema":"SOMA_RFC_ARCHIVE_OPERATION_PROJECTION_V1","scope_fingerprint":')
        digest.update(canonical_json_bytes(scope_fingerprint))
        digest.update(b',"scope_kind":')
        digest.update(canonical_json_bytes(scope_kind))
        digest.update(b"}")
        return digest.hexdigest(), exact_count
    ...
    def get_from_connection(
        ...
        *,
        ...
        scope_kind, scope_fingerprint = self._operation_header(connection, operation_id)
        projection_fingerprint, exact_count = self._stream_operation_projection_fingerprint(
            connection,
            operation_id=operation_id,
            scope_kind=scope_kind,
            scope_fingerprint=scope_fingerprint,
        )
        after = _validate_operation_cursor(
            cursor,
            operation_id=operation_id,
            projection_fingerprint=projection_fingerprint,
        )
