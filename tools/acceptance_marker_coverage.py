...
def _markers_for_packet(
    *,
    ...
    for path in sorted((repo_root / "tests").glob("test_*.py")):
        ...
        for marker in full_id.findall(text):
            evidence.setdefault(marker, set()).add(
                f"{relative}::<explicit-reference>"
            )

        for function_name in _test_functions(text):
...
def build_coverage(repo_root: Path, design_ref: str) -> dict[str, object]:
    ...
    return {
        "schema": "SOMA_PRE_LLD08_ACCEPTANCE_MARKER_COVERAGE_V1",
        "design_sha": resolved_design_sha,
        "implementation_sha": implementation_sha,
        "interpretation": (
            "Direct markers are a mechanical traceability aid only. "
