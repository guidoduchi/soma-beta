...
Status: **OPEN with F1–F3 and F7 corrected in HEAD; F4 and F6 open; F5 corrected in HEAD with bounded supersession pending; affected B002 F1/F7 assertions remain for explicit disposition; A2 section-destination map remains incomplete**  
...
| `RC006-A2-F3` | MEDIUM assurance-mechanics blocker | The pre-allocation RFC/WFM candidate corpus contained stored `fingerprint_sha256` values that did not consistently equal SHA-256 of the documented NFC/LF-normalized `exact_text`. | `A2-EXTRACTION-SCHEMA-V1` fingerprint rule; `RC006_A2_SCHEMA_AMENDMENT_001.md`; B002 RFC/WFM blob `433ffbea4fe291e451b0fefc2b2c8a999c1b1894` | Recomputed only fingerprint fields from unchanged reviewed `exact_text`, verified all 171 pinned-source spans and non-fingerprint fields, refroze the corpus, and allocated the verified range `A2-ASSERT-000654..000824`. | **CORRECTED / ALLOCATED** |
...
This finding does **not** reopen the RFC/WFM semantic review. The pinned source blob, exact text, source locators, section ordering, atomic boundaries, and reverse-authority dispositions remain independently reviewable. The repair was completed before RFC/WFM stable identity creation. All 171 source spans and non-fingerprint fields were preserved, the repaired corpus was frozen under the existing allocator contract, and the verified range `A2-ASSERT-000654..000824` was then allocated contiguously.
