...
|---|---|---|---|---|
| `FND-GAP-001` | The merged Alpha requirement catalogue has not yet been completely dispositioned, and unmerged Alpha proposals remain to be catalogued. | Open | Continue one-row-per-Alpha-ID review from `ALPHA:OPS-031`, then catalogue every unmerged proposal. Close only when no historical source item lacks a final disposition. | Roadmap Phase 0; Alpha Traceability Register |
