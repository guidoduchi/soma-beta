...
|---|---|---|---|---|
| `FND-GAP-001` | The merged Alpha requirement catalogue has not yet been completely dispositioned, and unmerged Alpha proposals remain to be catalogued. | Open | Continue one-row-per-Alpha-ID review from `ALPHA:OPS-030`, then catalogue every unmerged proposal. Close only when no historical source item lacks a final disposition. | Roadmap Phase 0; Alpha Traceability Register |
...
| `FND-GAP-005` | Earlier Spare Request wording risked treating the Huawei SR7 as the entity's creation identity and omitted local units without official provenance. | Resolved | `BETA-REQ-0014`–`0016` require local Spare Request origin with retained temporary identity and define optional-provenance `LSU-########` units. | Inventory Lifecycle Contract; Glossary; Decision D-009 |
