...
| Module | Owns |
|---|---|
| Identity and Settings | Local User Profile, Contacts and reusable reference configuration, installation configuration, appearance and built-in skins |
| Tickets | SRs, RFCs, hierarchy, ticket relationships, workbench Device Reference context, Contract Product Line classification, source observations |
| Objectives | Maintenance Windows, Local/WFM Tasks, RFC-derived context, operational Task plans, actual execution, Task outcomes, review, correction, cancellation, retry lineage, and Objective grouping |
| Inventory | Stock, BOM catalog, SR-level Spare Needs, Device Part Units, Spare Requests, RMA obligations, Spare Part Units, Task-to-unit allocations, reviewed maintenance physical consequences, requested and actual logistics, Fault Tag identities and memberships, pickup-origin snapshots, warehouse decisions, replacement/resend lineage, proposals, bulk actions, correction, and removal |
| Infrastructure | customer-owned Sites; Rooms/Racks; reusable Cloud Types and site-bound Deployments; Network Element models, instances, IP inventory, containment, placement, installed components, replacement history; Device Reference resolution/regularization; and reviewed workbook exchange |
...
| Overview and Reporting | curated metrics, narrative queues, filters, generated Excel reports, and immutable completed-report evidence |
...
| Audit | application audit of accepted commands, decisions, configuration changes, relationship mutations, and destructive-impact records; owning domains retain lifecycle-fact authority |
...
- WFM source planning from accepted operational Task planning, the derived Objective envelope from its member Task plans, and all planning layers from actual execution evidence;
...
- persistent Device References from the Network Elements to which they may later resolve without duplicating or rewriting their operational relationships;
- Device Part Units from Inventory Spare Part Units, even when one replaces the other;
- RMA obligation state from target assignment, direct inbound unit, reviewed Task outcome, accepted Inventory physical consequence, return-unit selection, Fault Tag, warehouse receipt, and final acceptance/rejection;
...
- current individual elapsed-time evidence and cohort SLA/reporting results from accepted facts and the latest Contract Product Line policy, while completed report evidence preserves the policy revision and results represented at completion.

Beta owns an independent migration lineage; Alpha migrations, ledgers, and table layouts are historical input and are never an executable starting point. The Beta persistence model explicitly separates import attempts, staging, accepted runs and compact observations, current projections, domain lifecycle evidence, general audit, report attempts, and completed report evidence. It contains no structure whose sole purpose is Alpha report finalization or purge authorization.
...
Reporting evaluates one consistent accepted-state view and must produce internally consistent immutable completed-report evidence once a report is successfully completed. Whether that evidence is persisted as an internal snapshot, reconstructed on demand from immutable inputs, or represented through both mechanisms remains the `O-001` LLD decision. Failed or cancelled generation retains only the bounded attempt evidence permitted by the reporting/audit contracts and cannot mutate operational state. Completed report evidence is historical authority, never cleanup authorization.
...
The operator may configure auto-accept only for explicitly safe source/change classes. High-risk classes in the product contract cannot be bypassed. Exact low-risk field/change classes remain the `O-007` LLD decision.
...
An unregistered Device Reference may first be recorded in an operational context and remains valid across Tickets, Objectives, and Inventory without Infrastructure registration. It may later be reviewably reconciled to exactly one existing Network Element or promoted into a newly created Network Element. Creation/promotion of a new Network Element uses the governed three-second deliberate hold; reconciliation to an existing Network Element follows its governed identity-resolution path. Both preserve the Device Reference identity and existing valid operational relationships and do not rewrite historical operational evidence.
...
Objective grouping is a domain policy, not a calendar-only UI behavior. It evaluates accepted operational Task intervals, prohibits accepted overlapping Objectives, stages consolidation when a Task bridges groups, and keeps Tasks without complete accepted intervals unscheduled. Source WFM planning remains evidence/candidate input rather than grouping authority until accepted into the operational Task plan.
...
An authoritatively accepted terminal Service Request transition may remove only that SR's direct Communication links. For an RFC, provider terminal evidence and import acceptance cannot unlink Communications; only the separately reviewed and confirmed local terminal cascade may remove that RFC's direct links. If either governed unlink removes the final protected operational dependency from a retained communication, the communication becomes **Orphaned — Pending Purge**. The positive installation-level grace period defaults to seven exact elapsed days. Restoring any protected dependency cancels pending purge. At the due time a housekeeping transaction revalidates dependencies before purging reconstructable message content. It preserves a non-reconstructable purge record, affected identities and chronology, and the frozen terminal summary. External PST/OST files, exported MSG files, portable exports, and existing backups are not modified. Reversal during grace restores access; reversal after purge may use targeted backfill when the source remains available and otherwise shows a coverage warning.

Service Request, Spare Request, and RFC workbenches project received/sent counts, direction, last-interaction age, coverage state, and safe navigation from canonical links without duplicating bodies. Terminal SR/RFC views keep a frozen minimal communication summary after their governed unlink/purge. Inventory communication matches may create idempotent reviewed proposals for supported Spare Request, RMA, logistics, Fault Tag, receipt, and final-decision milestones; final material actions remain subject to their owning domain confirmation rules.
...
1.0.0 includes all six workspaces, the stock-first Inventory, SR-level Need aggregation, Spare Request/RMA obligation and reviewed physical-consequence/Fault Tag and warehouse-loop lifecycles, pickup-origin and actual logistics, cross-request membership, correction replacement, rejection resend, state-specific removal, Objective/Task scheduling plus execution/outcome/review/correction/retry, Infrastructure identity/placement/IP inventory/containment, reviewed Infrastructure workbook import and discovery export, external request registration, reviewed proposals, compatible bulk actions, exact-event correction, security, official operational imports, Excel reporting, SLA, PST/OST read, MSG drafts, the complete responsive accessible UI/UX interaction and fixture system, atomic SQLite/migration/audit runtime, external redacted diagnostics, Windows/Python verification, and tray behavior.
