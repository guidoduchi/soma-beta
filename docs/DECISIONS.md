...
| D-016 | Contract Product Line classification and cohort-based SLA policy are core product capabilities. Each tier is a required cohort percentage within an inclusive duration, never a percentage of one ticket's deadline. IT and NFV defaults are confirmed; additional policies are Contract-configurable; Cancelled SRs are excluded; Alpha's global age-risk and suspension-KPI severity schedules do not control Beta 1.0. |
