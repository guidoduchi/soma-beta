...
| D-019 | 1.0.0 includes a responsive English UI, governed built-in skins, independent Light/Dark/System appearance, and Windows tray behavior. |
| D-020 | Excel is the 1.0.0 report export format. |
| D-021 | Ordinary operational and SLA calendar interpretation uses `America/Guayaquil`; weeks are Monday–Sunday and known instants persist as UTC whole seconds. Objective/Task schedule entry, presentation, and grouping use the separately selected IANA Objective timezone, while source adapters retain their accepted source-profile time rules. |
...
| D-024 | A Task is either Local or WFM. Local Tasks may link to zero or many SRs, master or subordinate RFCs, Spare Part Units, and Device References; a Device Reference may remain unregistered or resolve to one Network Element without replacing the Task relationship. A WFM retains exactly one RFC owner. |
...
| D-027 | For manual RFC/WFM/Objective records, hard deletion is limited to independently eligible untouched records with no import/adoption or protected operational evidence; other domains retain their own explicit deletion allowlists and otherwise use history-preserving lifecycle transitions. |
...
| D-039 | Weekly is the default ticket period with Daily/Monthly alternatives; terminal tickets then remain in Historical view. The configurable one-month default terminal-row lookback applies to Advanced Search under its accepted source-recency rule; RFC/WFM history is not removed by workbook age or omission. |
...
| D-049 | Once its inbox is configured, Advanced Search supports an enabled-by-default 10:00 daily check in fixed operational timezone `America/Guayaquil`, whole-minute configuration or disablement, one startup catch-up, and always-available manual checking. Scheduled invocation creates staging and cannot bypass review or safe-auto-accept policy. |
...
| D-152 | First run requires only Local Administrator authentication plus built-in skin, Light/Dark/System appearance, and Objective-timezone selection; operational master-data setup is skippable and missing classification yields no fabricated IT SLA fallback. |
...
| D-158 | The canonical SOMA brand assets (`soma-mark.svg`, `soma-wordmark.svg`, `soma-lockup.svg`, `soma.ico`, and `soma-tray.ico`) are owned by the project owner, may be used and distributed under SOMA Beta's proprietary/internal-use terms, and contain no third-party material requiring a surviving third-party license. Their Alpha origin remains recorded as provenance; Alpha's Apache-2.0 license does not become the licensing basis for those independently owned Beta assets or unrelated Beta material. |
