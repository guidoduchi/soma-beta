...
| Spare Request | A locally originated request and tracking container with an immutable temporary identity. It selects SR-level Spare Needs from exactly one Service Request, derives Customer Organization, records receiver and delivery/self-pickup logistics, and may later receive its official `SR` + 7 identifier. Requested quantity `N` may yield incremental `M ≤ N` C10 RMAs while pending or unfulfilled quantity remains explainable. |
...
| Fault Tag | A return-lifecycle container grouping physical return obligations and units from different Service Requests, Spare Requests, temporary tracking identifiers, and RMAs through warehouse receipt and final acceptance or rejection. |
